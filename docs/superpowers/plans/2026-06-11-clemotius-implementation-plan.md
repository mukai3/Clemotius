...
作成日: 2026-06-11
対象設計書: [2026-06-11-clemotius-design.md](../specs/2026-06-11-clemotius-design.md)
...
タスク:
- [ ] 0.1 .NET 8 ソリューション作成。`Clemotius`（WinForms, `OutputType=WinExe`, `TargetFramework=net8.0-windows`, `UseWindowsForms=true`）、`Clemotius.Core`（クラスライブラリ・Win32非依存ロジック）、`Clemotius.Tests`（xUnit）の3プロジェクト
- [ ] 0.2 `.gitignore`（`bin/`, `obj/`）、`Directory.Build.props`（`Nullable=enable`, `LangVersion=latest`, `<PlatformTarget>x64</PlatformTarget>`）
- [ ] 0.3 P/Invoke 集約用の `Clemotius/Interop/` 名前空間を用意（空でよい）

受け入れ基準: `dotnet build` 成功、`dotnet test` が0件でも成功で完走、`dotnet run --project Clemotius` で空のトレイ常駐プロセスが起動。
...
### 副作用層
- [ ] 3.4 `ConfigStore`: `%APPDATA%\Clemotius\config.json` 読み書き。初回は Default を書き出す。`FileSystemWatcher` で手動編集を即リロード（デバウンス付き）
...
- **挙動比較**: 同一入力をオリジナルと Clemotius に与え出力一致を確認（D.5）。差異は設計書に記録し許容/修正判断

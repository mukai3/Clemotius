# Clemotius [KazaguruMouse Alternative] 設計書

> アプリケーション名 **Clemotius** は、かざぐるまをモチーフとするオリジナルに対し、クレマチス（植物カザグルマの原種）をもじった命名。
...
| ConfigStore | `%APPDATA%\Clemotius\config.json` の読み書き。変更の即時反映（FileSystemWatcher） |
...
`%APPDATA%\Clemotius\config.json`。構造の骨子:
...
| `scroll.onVerticalScrollbar` | `none` | `ScrollExOnVScrlBar`=53(9行) | 垂直スクロールバー上の挙動。コード解読済み（垂直系=50..57 連番）。9行=垂直スクロール＝Clemotius では通常動作扱いで `none` |

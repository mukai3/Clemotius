...
新規コードは `src/Clemotius/SettingsUi/` 配下（既存 `Settings/` とは別フォルダ、プロジェクト分割なし）。
...
1. 各ViewModelは `ClemotiusConfig` の編集用コピー（既存 `MutableProfile` を流用）を保持
2. プロパティ変更 → ルート `SettingsViewModel` が `ClemotiusConfig` を再構築 → 現行と同じ `Applied` イベント → `ConfigStore.Save()`

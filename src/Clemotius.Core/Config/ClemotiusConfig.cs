...
public sealed record ClemotiusConfig
{
    ...
    /// <summary>設定画面の外観テーマ。"system"(OS連動) / "light" / "dark"。</summary>
    ...
    public static GestureProfile DefaultBrowserProfile() => new()
    {
        ...
        Gestures = new[]
        {
            ...
            new GestureBinding("DR", new AppCommandAction(AppCommand.BrowserClose)),     // ↓→ タブを閉じる(APPCOMMAND_CLOSE。CloseAction=WM_CLOSEだとウィンドウごと閉じる)

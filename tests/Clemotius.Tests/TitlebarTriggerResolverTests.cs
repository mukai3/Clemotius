...
public class TitlebarTriggerResolverTests
{
    ...
    {
        var config = new Clemotius.Core.Config.ClemotiusConfig
        {
            Titlebar = Settings with { WindowOpacity = 70 },
        };
        var restored = Clemotius.Core.Config.Json.ConfigSerializer.Deserialize(
            Clemotius.Core.Config.Json.ConfigSerializer.Serialize(config));
